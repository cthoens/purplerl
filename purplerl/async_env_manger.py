...
class ProcessEnvManager(EnvManager):
    def __init__(self, env_name='CartPole-v0', num_envs=2) -> None:
        self.shared_obs = torch.zeros(num_envs, *WorkbookEnv.OBSERVATION_SPACE.shape, dtype=torch.float32).share_memory_()
        ...
        self.observation_space = WorkbookEnv.OBSERVATION_SPACE
        self.action_space = WorkbookEnv.ACTION_SPACE
    ...
    def step(self, actions: np.ndarray):
        ...
        success = np.array([info.get("success", None) for _, _, info in interaction], dtype=np.float32)
